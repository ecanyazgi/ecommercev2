...
def test_inventory_category_dbfixture(db, db_fixture_setup, id, name, slug, is_active):
    # db grants us access to database in parameters
...
def test_inventory_db_brand_insert_data(db, brand_factory):
    ...
    assert new_brand.name == "brand_1"


def test_inventory_db_brand_unique(db, brand_factory):
    new_brand = brand_factory.create(name="not_unique")
    with pytest.raises(IntegrityError):
        not_new_brand = brand_factory.create(name="not_unique")


def test_inventory_db_product_type_insert_data(db, product_type_factory):
    new_product_type = product_type_factory.create()
    print(new_product_type.name)
    assert new_product_type.name == "type_1"


def test_inventory_db_type_unique(db, product_type_factory):
    new_brand = product_type_factory.create(name="not_unique")
    with pytest.raises(IntegrityError):
        not_new_brand = product_type_factory.create(name="not_unique")
...
def test_inventory_db_media_insert_data(db, media_factory):
    new_media = media_factory.create(product_inventory__id="34465")
    print(new_media.product_inventory.id)
    assert new_media.product_inventory.sku == "sku_1"
...
def test_inventory_db_stock_dataset(
        db, db_fixture_setup, id, product_inventory, last_checked, units, units_sold
):
    result = models.Stock.objects.get(id=id)
    result.last_checked = result.last_checked.strftime("%Y-%m-%d %H:%M:%S")
    assert result.id == id
    assert result.product_inventory.id == product_inventory
...
def test_inventory_db_stock_insert_data(db, stock_factory):
    new_stock = stock_factory.create(product_inventory__id="34465")
    assert new_stock.product_inventory.id == 8617
